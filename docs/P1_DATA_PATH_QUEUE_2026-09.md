...
| **Q6** | P1-C | **DRIP not compounding** in backfill; **DRIP not atomic**; sold-position dividend attribution symbol-wide | Review portfolio P1s | **Bolt** | **Erick** for lot model | First slice: payment-order share updates + CAS/unclaim `processDRIP` (this cycle). Sold totals remain symbol/date-range until Q7 lot model. Deterministic tests. |
